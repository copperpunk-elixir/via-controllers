defmodule ViaControllers.FixedWing.Tecs.Balance do
  ...
  def update(tecs, cmds, values, _airspeed, dt_s) do
    ...
    # Logger.debug("tecs bal: #{ViaUtils.Format.eftb_deg(output,1)}")
    # Logger.debug("p/i/d/rate/total: #{ViaUtils.Format.eftb_deg(cmd_p,3)}/#{ViaUtils.Format.eftb_deg(cmd_i,3)}/#{ViaUtils.Format.eftb_deg(cmd_d, 3)}/#{ViaUtils.Format.eftb(cmd_rate,3)}/#{ViaUtils.Format.eftb_deg(output, 3)}")
